...
class SutureBranchAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        try {
            val output = service.branch(baseDir)
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
                .createNotification("Suture Branches", output.ifBlank { "No branches found." }, NotificationType.INFORMATION)
                .notify(project)
        } catch (e: Exception) {
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
