...
class SuturePullAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        try {
            val output = service.pull(repoRoot)
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
                .createNotification("Suture Pull", output.ifBlank { "Pulled successfully." }, NotificationType.INFORMATION)
                .notify(project)
        } catch (e: Exception) {
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
