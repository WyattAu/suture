...
class SuturePushAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        try {
            val output = service.push(repoRoot)
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
                .createNotification("Suture Push", output.ifBlank { "Pushed successfully." }, NotificationType.INFORMATION)
                .notify(project)
        } catch (e: Exception) {
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
