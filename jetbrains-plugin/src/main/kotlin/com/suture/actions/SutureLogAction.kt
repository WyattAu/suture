...
class SutureLogAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        try {
            val output = service.log(baseDir)
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
                .createNotification("Suture Log", output.ifBlank { "No commits found." }, NotificationType.INFORMATION)
                .notify(project)
        } catch (e: Exception) {
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
