...
class SutureInitAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        try {
            val output = service.init(baseDir)
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
                .createNotification("Suture Init", output.ifBlank { "Repository initialized successfully." }, NotificationType.INFORMATION)
                .notify(project)
        } catch (e: Exception) {
            NotificationGroupManager.getInstance()
                .getNotificationGroup("Suture Notifications")
